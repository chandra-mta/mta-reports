...
#
# --- hrc_data_set.py extract hrc data, compute stats, and plot values
# --- author w. aaron (william.aaron@cfa.harvard.edu)
# --- last update: Nov 04, 2024
#

#
# --- Note, as of 2020, ephin data in the sci_run_interruption was replaced with records of
# --- HRC shield event rates at time of interruption. This script is designed to be a submodule of run_interruption.py
#
import os
import numpy as np
...
def hrc_data_set(event_data, pathing_dict):
    """
    Collect together HRC data for plotting, stats, and storage
    """
    fetch_result = fetch.MSIDset(MSIDS, start = event_data['tstart'] - timedelta(days = 2), stop = event_data['tstop'] + timedelta(days = 2), stat='5min')
...
def write_hrc_files(fetch_result, event_data, pathing_dict):
    ...
    # --- Write Data File
    #
    line = HRC_DATA_HEADER.replace("#LSTART",event_data['tstart'].strftime(TIME_FORMAT))
    times = convert_time_format(fetch_result[MSIDS[0]].times, fmt_in='secs', fmt_out = 'date')
    vals_group = []
    for msid in MSIDS:
    ...
    ifile = os.path.join(pathing_dict['OUT_WEB_DIR'], 'Data_dir', f"{event_data['name']}_hrc.txt")
    ifile2 = os.path.join(pathing_dict['OUT_WEB_DIR2'], 'Data_dir', f"{event_data['name']}_hrc.txt")
    ...
    ifile = os.path.join(pathing_dict['OUT_WEB_DIR'], 'Stat_dir', f"{event_data['name']}_hrc_stat")
    ifile2 = os.path.join(pathing_dict['OUT_WEB_DIR2'], 'Stat_dir', f"{event_data['name']}_hrc_stat")
    os.makedirs(os.path.dirname(ifile), exist_ok= True)
    os.makedirs(os.path.dirname(ifile2), exist_ok= True)
    with open(ifile,'w') as f:
        f.write(line)
...
def plot_hrc_data(fetch_result, event_data, pathing_dict):
    """
    Create a plot of HRC data during the 
    """
    rad_zones = events.rad_zones.filter(start = event_data['tstart'] - timedelta(days = 2), stop = event_data['tstop'] + timedelta(days = 2)).table
    ...
    plt.text(event_data['tstart'] + (0.01 * deltatime), 4.6, r'Interruption', color='red') # Interruption Marker
    ofile = os.path.join(pathing_dict['OUT_WEB_DIR'], 'HRC_plot', f"{event_data['name']}_hrc.png")
    ofile2 = os.path.join(pathing_dict['OUT_WEB_DIR2'], 'HRC_plot', f"{event_data['name']}_hrc.png")
